...
int main() {
    ...
    while (1) {
        ...
        // Converte os valores brutos para valores legíveis usando os dados de calibração
        int32_t compensated_temp = bmp280_convert_temp(raw_temp, &bmp280_params);
        int32_t compensated_pressure = bmp280_convert_pressure(raw_pressure, raw_temp, &bmp280_params);

        printf("BMP280 -> Temp: %.2f C, Pressao: %.2f hPa\n", compensated_temp / 100.0, compensated_pressure / 100.0);
