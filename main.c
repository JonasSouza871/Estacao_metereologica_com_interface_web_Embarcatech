...
/* ---- Variáveis Globais ---------------------------------------- */
volatile uint8_t tela_atual = TELA_ABERTURA; // Indica qual tela está sendo exibida
volatile bool    atualizar_tela_flag = true; // Sinaliza se a tela precisa ser atualizada
volatile float   fator_zoom = 1.0f; // Fator de zoom para o gráfico de temperatura

static uint64_t  ultimo_zoom_ms = 0; // Última vez que o zoom foi ajustado (em milissegundos)
const  uint32_t  ZOOM_DEBOUNCE_MS = 120; // Tempo mínimo entre ajustes de zoom

float buffer_temperaturas[TAMANHO_GRAFICO]; // Armazena as últimas temperaturas medidas
int   indice_buffer = 0; // Índice atual no buffer circular
int   contador_buffer = 0; // Conta quantas leituras foram armazenadas
...
/* ================================================================ */
int main() {
    stdio_init_all(); // Inicializa comunicação padrão (ex.: USB)
    sleep_ms(2000); // Aguarda 2 segundos para estabilizar

    ssd1306_t tela; // Estrutura para o display OLED
    struct bmp280_calib_param parametros_bmp; // Parâmetros de calibração do BMP280
    float temperatura_aht = 0, temperatura_bmp = 0, temperatura_media = 0, umidade = 0, pressao = 0;
    uint64_t proxima_leitura = 0; // Controla o tempo da próxima leitura dos sensores

    configurar_perifericos(&tela, &parametros_bmp); // Configura I2C, sensores e tela
    configurar_botoes(); // Configura os botões com interrupções
    configurar_joystick(); // Configura o joystick para zoom

    while (true) {
        // Lê os sensores a cada INTERVALO_LEITURA_MS (2 segundos)
        if (time_us_64() >= proxima_leitura) {
            ...
            // Armazena a temperatura média no buffer circular
            ...
            // Atualiza a tela se estiver nas telas de valores ou gráfico
        ...
        callback_joystick(); // Verifica o joystick para ajustar o zoom
...
void configurar_perifericos(ssd1306_t *tela, struct bmp280_calib_param *parametros) {
    // Inicializa I2C para sensores (frequência de 100 kHz)
    ...
    // Inicializa I2C para tela (frequência de 400 kHz)
    ...
    // Inicializa os sensores AHT20 (temperatura e umidade) e BMP280 (temperatura e pressão)
...
void configurar_joystick(void) {
    ...
    adc_gpio_init(JOYSTICK_PIN); // Configura o pino do joystick
...
void callback_botoes(uint gpio, uint32_t eventos) {
    ...
    if (eventos & GPIO_IRQ_EDGE_FALL) {
        if (gpio == BOTAO_A_PIN && agora - ultimo_pressionamento_a > DEBOUNCE_MS * 1000) {
            tela_atual = (tela_atual + 1) % 4; // Avança para a próxima tela
            atualizar_tela_flag = true;
            ultimo_pressionamento_a = agora;
        }
        if (gpio == BOTAO_B_PIN && agora - ultimo_pressionamento_b > DEBOUNCE_MS * 1000) {
            tela_atual = (tela_atual == 0) ? 3 : tela_atual - 1; // Volta para a tela anterior
            atualizar_tela_flag = true;
...
void callback_joystick(void) {
    ...
    uint16_t valor = adc_read(); // Lê o valor analógico do joystick
    uint64_t agora = to_ms_since_boot(get_absolute_time());
    if (agora - ultimo_zoom_ms < ZOOM_DEBOUNCE_MS) return;

    bool mudou = false;
    if (valor > ZONA_MORTA_ALTA && fator_zoom < 4.0f) { // Zoom in
        fator_zoom += 0.10f;
        mudou = true;
    } else if (valor < ZONA_MORTA_BAIXA && fator_zoom > 0.25f) { // Zoom out
        fator_zoom -= 0.10f;
        mudou = true;
    }
    if (mudou) {
...
void mostrar_tela_abertura(ssd1306_t *tela) {
    ssd1306_fill(tela, 0); // Limpa o display
    ssd1306_rect(tela, 5, 5, 118, 54, 1, false); // Desenha um retângulo
...
void mostrar_tela_grafico(ssd1306_t *tela) {
    ...
    /* Área do gráfico */
    const uint8_t gx = 20;  // Posição X do eixo
    const uint8_t gy = 52;  // Posição Y do eixo
    const uint8_t H  = 40;  // Altura do gráfico
    const uint8_t W  = 105; // Largura do gráfico
    ...
    /* Calcula o passo para as marcações do eixo Y */
    ...
    ssd1306_vline(tela, gx, gy, gy + 2, true); // 0s
    ssd1306_vline(tela, gx + W / 2, gy, gy + 2, true); // 30s
    ssd1306_vline(tela, gx + W, gy, gy + 2, true); // 60s
